# source(here(nemo_path,"scripts/functions/R_workspace.R")) # run this script within R environment
...
# mus_musculus.gtf <- import(here(nemo_path,"/genomes/ensembl/Mus_musculus.GRCm38.101.gtf")) %>% as_tibble
# saveRDS(mus_musculus.gtf, here(nemo_path,"/genomes/ensembl/Mus_musculus.GRCm38.101.gtf.RDS"))
# mus_musculus.gtf <- readRDS(here(nemo_path,"/genomes/ensembl/Mus_musculus.GRCm38.101.gtf.RDS"))
# mus.musculus.gene2ens <- mus_musculus.gtf %>% dplyr::select(gene_id, gene_name) %>% unique()
# saveRDS(mus.musculus.gene2ens, here(nemo_path,"/genomes/ensembl/Mus_musculus.GRCm38.101.gene2ens.RDS"))
mus.musculus.gene2ens <- readRDS(here(nemo_path,"/genomes/ensembl/Mus_musculus.GRCm38.101.gene2ens.RDS")) %>% mutate(gene_name.mouse = gene_name, gene_name = toupper(gene_name))
# mus.musculus.tx2gene <- mus_musculus.gtf %>% dplyr::select(transcript_id, gene_id) %>% unique() %>% drop_na()
# saveRDS(mus.musculus.tx2gene, here(nemo_path,"/genomes/ensembl/Mus_musculus.GRCm38.101.tx2gene.RDS"))
mus.musculus.tx2gene <- readRDS(here(nemo_path,"/genomes/ensembl/Mus_musculus.GRCm38.101.tx2gene.RDS"))

# homo_sapiens.gtf <- import(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gtf")) %>% as_tibble
# transcript.gtf <- import(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gtf")) %>% as_tibble %>% distinct(transcript_id, .keep_all = TRUE) %>% mutate(coord = paste0("chr", seqnames, ":", start, "-", end))
# phast <- phastCons100way.UCSC.hg38
gtf <- readRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gtf.coords.RDS"))
...
gene2ens <- readRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gene2ens.RDS")) %>% unique
tx2gene <- readRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.chr_patch_hapl_scaff.tx2gene.rds")) %>% rename("transcript_id" = TXNAME, "gene_id" = GENEID) %>% unique # data.frame with 2 columns: ENST ID & ENSG ID
# homo_sapiens.gtf %>% dplyr::select(transcript_id, gene_name) %>% drop_na %>% unique %>% saveRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.enst2gene.rds"))
# enst2gene <- readRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.enst2gene.rds")) # data.frame with 2 columns: ENST ID & ENSG ID
# ensembl_transcript_biotype <- readRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.transcript_biotype.rds"))
# ensembl_gene_biotype <- readRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gene_biotype.rds"))


# GO terms ----------------------
# gmtPathways() # reads in as vector read.gmt() # reads in as dataframe
gprofiler_database <- gmtPathways(here(nemo_path,"/genomes/gene-ontology/gprofiler_full_hsapiens.name.edit.gmt")) # IDs removed to see term names
# https://www.gsea-msigdb.org/gsea/msigdb/collections.jsp GO pathways
# curated.gene.sets.msigdb <- read.gmt(here(nemo_path,"/genomes/gene-ontology/c2.all.v7.1.symbols.gmt")) # includes KEGG, REACTOME, PID, BIOCARTA, CGP
kegg.pathways.msigdb <- gmtPathways(here(nemo_path,"/genomes/gene-ontology/c2.cp.kegg.v7.1.symbols.gmt")) #%>% mutate(term = gsub("KEGG|_", " ", term))
# reactome.pathways.msigdb <- read.gmt(here(nemo_path,"/genomes/gene-ontology/c2.cp.reactome.v7.1.symbols.gmt")) %>% mutate(term = gsub("REACTOME|_", " ", term))
go.pathways.msigdb <- gmtPathways(here(nemo_path,"/genomes/gene-ontology/c5.all.v7.0.symbols.gmt"))# %>% mutate(term = gsub("GO|_", " ", term))
# go.bp.pathways.msigdb<- gmtPathways(here(nemo_path,"/genomes/gene-ontology/c5.bp.v7.1.symbols.gmt"))# %>% mutate(term = gsub("GO|_", " ", term))
# go.mf.pathways.msigdb<- gmtPathways(here(nemo_path,"/genomes/gene-ontology/c5.mf.v7.1.symbols.gmt"))# %>% mutate(term = gsub("GO|_", " ", term))
# go.cc.pathways.msigdb <- gmtPathways(here(nemo_path,"/genomes/gene-ontology/c5.cc.v7.1.symbols.gmt"))# %>% mutate(term = gsub("GO|_", " ", term))
hallmarks.msigdb <- read.gmt(here(nemo_path,"/genomes/gene-ontology/h.all.v7.4.symbols.gmt")) %>% mutate(term = gsub("HALLMARK|_", " ", term))
# immune.pathways.msigdb <- read.gmt(here(nemo_path,"/genomes/gene-ontology/c7.all.v7.1.symbols.gmt"))
...
RNA_BINDING_PROTEINS <- read_excel(here(nemo_path,"/genomes/gene-ontology/rbp_consensus_gerstberger_2014.xls"), sheet = "RBP table") %>% dplyr::pull(`gene name`)
...
# mouse2human = readRDS(here(nemo_path,"/genomes/ensembl/mouse2human.rds"))
...
theme_oz <- function (xaxis_arrow = TRUE, text_size = 8) { 
  if(xaxis_arrow == TRUE) {
  theme_bw(base_size=text_size) %+replace% # theme_bw(base_size=8, base_family="Helvetica") %+replace% 
    theme(
  ...
  if(!is.null(plot.path.prefix)){
   write_csv(res, file = paste0(metadata$database_dir[1],"/expression/deseq2/results/",plot.path.prefix,"_deseq_results.csv"), na = "")
    if(plot.figures == TRUE){
      ...
      write_csv(progeny.contrast_acts, file = paste0(metadata$database_dir[1],"/expression/deseq2/results/",plot.path.prefix,"_progeny_results.csv"), na = "")
...
track_viewer_nuc_cyt <- function(level = "gene", gene_name = NULL, transcript_id = NULL, cond1.nuc.bam = ctrl_nuclear.bam, cond1.cyt.bam = ctrl_cytoplasm.bam, cond1.label = "CTRL", cond2.cyt.bam = NULL, cond2.nuc.bam = NULL, cond2.label = "MUT", 
  ...
  if(!is.null(clip_track)) { # plot CLIP peaks
    if(file.exists(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/HepG2/"), clip_track, ".merged.bed.gz"))){ print(paste0(clip_track, " clip bed found in HepG2"))
      CLIP <- import(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/HepG2/"), clip_track, ".merged.bed.gz"), "BED")
    }else if(file.exists(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/K562/"), clip_track, ".merged.bed.gz"))){ print(paste0(clip_track, " clip bed found in K562"))
      CLIP <- import(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/K562/"), clip_track, ".merged.bed.gz"), "BED")
    } else if(file.exists(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/Hela_hg19/xlinks/"), clip_track, ".bed"))){ print(paste0(clip_track, " clip bed found in Hela_hg19. liftOver to hg38."))
      CLIP <- import(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/Hela_hg19/xlinks/"), clip_track, ".bed"), "BED") # hg19
      # hg38toHg19 for Hela
      require(rtracklayer) # https://genviz.org/module-01-intro/0001/06/02/liftoverTools/
      ch = import.chain(here(nemo_path,"genomes/rbp-databases/clip-bedgraphs/hg19ToHg38.over.chain")) # import chain for liftOver http://hgdownload.soe.ucsc.edu/goldenPath/hg38/liftOver/hg38ToHg19.over.chain.gz
...
# homo_sapiens.gtf <- import(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gtf")) %>% as_tibble
# saveRDS(homo_sapiens.gtf, here(nemo_path, "genomes/ensembl/Homo_sapiens.GRCh38.99.rds"))
...
# saveRDS(gtf.apa, here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gtf.apa.coords.rds"))
# gtf.apa <- readRDS(here(nemo_path,"/genomes/ensembl/Homo_sapiens.GRCh38.99.gtf.apa.coords.rds"))
...
track_viewer_nuc_cyt_clip <- function(gene.name = "FMR1", mut.nuc.bam = mn_nuc_vcp.bam, ctrl.nuc.bam = mn_nuc_ctrl.bam, mut.cyt.bam = mn_cyt_vcp.bam, ctrl.cyt.bam = mn_cyt_ctrl.bam,
  ...
  # Import iCLIP peaks
  if(file.exists(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/HepG2/"), gene.name, ".merged.bed.gz"))){
    print(paste0(gene.name, " clip bed found in HepG2"))
    CLIP <- import(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/HepG2/"), gene.name, ".merged.bed.gz"), "BED")
  }else if(file.exists(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/K562/"), gene.name, ".merged.bed.gz"))){
    print(paste0(gene.name, " clip bed found in K562"))
    CLIP <- import(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/K562/"), gene.name, ".merged.bed.gz"), "BED")
  } else if(file.exists(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/Hela_hg19/"), gene.name, ".merged.bed.gz"))){
    print(paste0(gene.name, " clip bed found in Hela_hg19"))
    CLIP <- import(paste0(here(nemo_path,"/genomes/rbp-databases/clip-bedgraphs/Hela_hg19/"), gene.name, ".merged.bed.gz"), "BED")
  }
  seqlevelsStyle(CLIP) <- "NCBI"
  CLIP <- new("track", dat=CLIP, type="data", format="BED")
...
track_viewer <- function(gene_test = "SFPQ", txt = ac_who_vcp_vs_ctrl, mut.bam = ac_who_vcp, ctrl.bam = ac_who_ctrl, xpad = 1000){
  ...
  # save figure
  png(file = paste(here(nemo_path,"/projects/astrocyte-ir-als/splicing/IRFinder/figures/coverage/"), gene_test, "_coverage_plot.png", sep = ""), height = 3.5, width = 10, units ="in", res = 300)
  vp <- viewTracks(trackList, gr=gr, viewerStyle=viewerStyle)
...
track_viewer_tx <- function(gene_test = "RECK", txt = ac_who_vcp_vs_ctrl, mut.bam = ac_who_vcp, ctrl.bam = ac_who_ctrl, path = NULL){
  ...
  print("Saving figure.")
  png(file = paste(here(nemo_path,"/projects/astrocyte-ir-als/splicing/IRFinder/figures/coverage/", path, gene_test, "_tx_coverage_plot.png", sep = "")), height = 2.2, width = 7, units ="in", res = 300)
  vp <- viewTracks(trackList, gr=gr, viewerStyle=viewerStyle)
...
track_viewer_tx_top_IRratio_event <- function(gene_test = "RECK", txt = ac_who_vcp_vs_ctrl, mut.bam = ac_who_vcp, ctrl.bam = ac_who_ctrl, path = NULL){
  ...
  print("Saving figure.")
  png(file = paste(here(nemo_path,"/projects/astrocyte-ir-als/splicing/IRFinder/figures/coverage/"), path, gene_test, "_topDeltaIRratio_tx_coverage_plot.png", sep = ""), height = 2.2, width = 7, units ="in", res = 300)
  vp <- viewTracks(trackList, gr=gr, viewerStyle=viewerStyle)
  addGuideLine(c(st, en), vp=vp, col = "firebrick2", lwd = 2, lty = "dashed")
  grid.text(IRratio_ctrl, x=.57, y=.65, just="bottom")
  grid.text(IRratio_vcp, x=.57, y=.2, just="bottom")
...
track_viewer_tx_multiple <- function(gene_test = "SFPQ", txt = ac_who_vcp_vs_ctrl, mut.bam = ac_who_vcp, ctrl.bam = ac_who_ctrl, path = NULL){
  ...
  print("Saving figure.")
  png(file = paste(here(nemo_path,"/projects/astrocyte-ir-als/splicing/IRFinder/figures/coverage/"), path, gene_test, "_tx_multiple_coverage_plot.png", sep = ""), height = 2.5, width = 8, units ="in", res = 300)
  vp <- viewTracks(trackList, gr=gr, viewerStyle=viewerStyle)
  for (i in 1:nrow(events)) {
    addGuideLine(c(st[[i]], en[[i]]), vp=vp, col = "firebrick2", lwd = 1, lty = "dashed")
    # grid.text(IRratio_ctrl[[i]], x=.545, y=.6, just="bottom")
...
# saveRDS(uniprot2gene, here(nemo_path, "genomes/annotation/uniprot2gene.rds"))
